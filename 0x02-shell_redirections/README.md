...
- [Shell, I/O Rdirection](http://linuxcommand.org/lc3_lts0070.php)
...
Write a script that writes into the file `ls_cwd_content` the result of the cmmand `ls -la`. If the file `ls_cwd_content` already exists, it should be overwritten. If the file `ls_cwd_content` does not exist, create it.
...
Write a script that deletes all the regular files (not the directories) with a `.js` extension that are present in the current dierectory and all its subfolders.
...
Dispaly lines containing the pattern "root" from the file `/etc/passwd`
